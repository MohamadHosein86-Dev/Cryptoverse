...
const CoinDetails = () => {
  ...
  const [coin, setCoin] = useState<CoinDetailsObjectType | undefined>(undefined);
  ...
  const handleDaysChange = async (event: SelectChangeEvent<number | string>) => {
    if (coinId) {
      setLoading(true);
      setDays(event.target.value);
      const prices = await getPrices(coinId, event.target.value, priceType, setError);
      if (prices) {
        settingChartData(setChartData, prices);
        setLoading(false);
      }
  ...
  const handlePriceTypeChange = async (_event: MouseEvent<HTMLElement>, newPriceType: string) => {
      ...
      {!loading && !error && coin !== undefined ? (
        ...
        <div className="p-4 2xl:w-1/3 sm2:w-3/5 max-sm:w-full">
          <div className="p-4 rounded-xl bg-[#0D0D0D] text-red-500 text-center text-sm">
            {error} - Error while loading {coinId} details. Please try again later
          </div>
        </div>
      ) : (
        <LoaderSpinner />
